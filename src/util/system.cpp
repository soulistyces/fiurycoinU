...
const char * const BITCOIN_CONF_FILENAME = "fiurycoin.conf";
...
#else
    const char* pszModule = "fiurycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fiurycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fiurycoin";
#else
    // Unix
    return pathRet / ".fiurycoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
        ...
        strCopyrightHolders += "\n" + strYear + "The Fiurycoin Core developers";
