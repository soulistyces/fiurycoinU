...
#if defined(NDEBUG)
# error "Fiurycoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Fiurycoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s FIURY additional fees, %d delta bytes\n",
...
CAmount nSubsidy;
    if (nHeight == 1 || nHeight == 2 || nHeight == 3 || nHeight == 4 || nHeight == 5 || nHeight == 6)  {
        nSubsidy = 59500 * COIN;   // premine 6x59500 coins
    } else {
    nSubsidy = 27 * COIN;
	 }
...
void ThreadScriptCheck() {
    RenameThread("fiurycoin-scriptch");
